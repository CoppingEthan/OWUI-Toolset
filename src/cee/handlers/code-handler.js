/**
 * Code File Handler
 * Reads source code files and returns content with syntax highlighting hints
 */

import { getFileTypeDescription, formatFileSize, getCodeLanguage } from '../../utils/file-type-detector.js';
 ...
 */
export async function extractCode(fileBuffer, filename, metadata) {
  const fileType = getFileTypeDescription(filename);
  const fileSize = formatFileSize(fileBuffer.length);
  const language = getCodeLanguage(filename) || 'text';
