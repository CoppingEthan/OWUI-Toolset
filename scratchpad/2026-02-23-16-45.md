...
**Chat endpoint message handling (was lines 912-935, now lines 927-1010):**
...
5. Sends an SSE status event to notify the user when their message is truncated

**User-facing status notification (inside truncation block, both string and multimodal paths):**
When the latest user message is truncated, a status event is sent via `sendSSEEvent()`:
```
✂️ Your message was too long (~X tokens) and has been truncated
```
Uses the same pattern as compaction status (`done: true` = flash notification, not a spinner). Only fires for streaming requests. SSE headers are already set up before the truncation block runs (line 918).

**JSDoc comment (was lines 1410-1414, now lines ~1495):**
