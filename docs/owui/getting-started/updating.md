...
Before Updating
  ...
  * **Running Multiple Workers?** If you use `UVICORN_WORKERS > 1`, you **MUST** ensure migrations run safely by either:
    ...
    2. Designating a master worker using `ENABLE_DB_MIGRATIONS=True` (default) on one instance and `False` on others. Once migrations complete, you can run with multiple workers normally.



Help Us Test Before Release
...
[Edit this page](https://github.com/open-webui/docs/blob/main/docs/getting-started/updating.mdx)

[PreviousEnvironment Variable Configuration](/getting-started/env-configuration)[NextAPI Endpoints](/getting-started/api-endpoints)
