...
Do Not Share Data with Production
...
Before you begin, ensure your system meets these minimum requirements:
  ...
  * **Operating System:** Linux (or WSL on Windows), Windows 11, or macOS. _(Recommended for best compatibility)_
  * **Python:** Version **3.11 or higher**. _(Required for backend services)_
  * **Node.js:** Version **22.10 or higher**. _(Required for frontend development)_
...
_Note: Depending on your Open WebUI version, you might see compatibility warnings or errors. If so, just run:_
           
           npm install --force  
           

_Some setups need this to get around version issues._
...
VS Code's integrated terminal feature makes managing multiple terminals incredibly easy. Here's how to leverage it for frontend and backend separation:
  ...
  2. **Backend Terminal (Open a New One):**
     ...
     * **Navigate to the`backend` directory:** In this _new_ terminal, use the `cd backend` command to change the directory to the `backend` folder within your project. This ensures all backend-related commands are executed in the correct context.
...
**Backend Setup Steps (in your _backend_ terminal):**

  1. **Navigate to the Backend Directory:** (You should already be in the `backend` directory in your _new_ terminal from the previous step). If not, run:
...
_(Using Conda is optional but strongly recommended for managing Python dependencies and avoiding conflicts.)_ If you choose not to use Conda, ensure you are using Python 3.11 or higher and proceed to the next step, but be aware of potential dependency conflicts.

  3. **Alternative: Create and Activate a Python venv:**
...
Once activated, your terminal prompt should show `(venv)`. You can now proceed to install dependencies.
  ...
  4. **Install Backend Dependencies:**
     ...
     * In your _backend_ terminal (and with the Conda environment activated if you are using Conda), run:
...
This command uses `pip` (Python Package Installer) to read the `requirements.txt` file in the `backend` directory. `requirements.txt` lists all the Python libraries that the backend needs to run. `pip install` downloads and installs these libraries into your active Python environment (your Conda environment if you are using it, or your system-wide Python environment otherwise). The `-U` flag ensures you get the latest compatible versions of the libraries.
  ...
  5. **Start the Backend Development Server:**
     ...
     * In your _backend_ terminal, run:
    
    sh dev.sh  
    

This command executes the `dev.sh` script. This script likely contains the command to start the backend development server. _(You can open and examine the`dev.sh` file in your code editor to see the exact command being run if you are curious.)_ The backend server will usually start and print some output to the terminal.
...
**Solution:** Increase the Node.js heap size. This gives Node.js more memory to work with. You have a couple of options:
  ...
  1. **Using`NODE_OPTIONS` Environment Variable (Recommended for Development):**
     ...
     * This is a temporary way to increase the memory limit for the current terminal session. Before running `npm run dev` or `npm run build` in your _frontend_ terminal, set the `NODE_OPTIONS` environment variable:
...
**Troubleshooting Steps:**
  ...
  4. **Dependency Issues (Frontend):** Outdated or corrupted frontend dependencies can sometimes interfere with hot reloading. Try refreshing your frontend dependencies:
     * In your _frontend_ terminal, run:
...
[Edit this page](https://github.com/open-webui/docs/blob/main/docs/getting-started/advanced-topics/development.md)

[PreviousAdvanced Topics](/getting-started/advanced-topics/)[NextNetwork Diagrams](/getting-started/advanced-topics/network-diagrams)
