...
Now that the model is downloaded, the next step is to run it using **Llama.cpp’s server mode**. Before you begin:
  ...
  1. **Locate the`llama-server` binary.** If you built from source (as outlined in Step 1), the `llama-server` executable will be located in `llama.cpp/build/bin`. Navigate to this directory by using the `cd` command:
...
Replace `[path-to-llama-cpp]` with the location where you cloned or built Llama.cpp. For example:
  ...
  2. **Point to your model folder.** Use the full path to the downloaded GGUF files created in Step 2. When serving the model, specify the first part of the split GGUF files (e.g., `DeepSeek-R1-UD-IQ1_S-00001-of-00003.gguf`).
...
![Server Screenshot](/assets/images/serve-36ee8710486caffd9120156034449f39.png)
...
![Connection Screenshot](/assets/images/connection-2190bc995ae9112d4762349d7b17764d.png)
...
![Response Screenshot](/assets/images/response-45defb791d848193082c59f66d937bd7.png)
...
[Edit this page](https://github.com/open-webui/docs/blob/main/docs/tutorials/integrations/deepseekr1-dynamic.md)

[PreviousWorkload Identity Authentication](/tutorials/integrations/azure-openai/workload-identity-auth)[NextBackend-Controlled, UI-Compatible API Flow](/tutorials/integrations/backend-controlled-ui-compatible-flow)
